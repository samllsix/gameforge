...
def test_preview_task_id_resolves_sandbox_path(api_client, tmp_path, monkeypatch):
    """sandbox task_id 应解析到 DATA_ROOT/sandbox/<project_id>/tasks/<task_id>/

    路径经 src.core.paths.sandbox_task_dir 收口（DATA_ROOT 锚定，cwd 无关）。
    """
    import os
    ...
    assert resolved.endswith(os.path.join("sandbox", project_id, "tasks", task_id))
